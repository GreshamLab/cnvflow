...
Three replicate populations of the GAP1 CNV reporter strain, in which a mCitrine gene was inserted upstream of the GAP1 promoter and coding sequence, was experimentally evolved in chemostats in glutamine-limited media for 200 generations. Additionally, there was 1 population each of 3 control strains: zero copy 'unstrained' control (DGY1) that has no mCitrine, one copy control (DGY500) in which the mCitrine gene was inserted in neutral locus, *WHAT LOCUS?* that doesn't undergo copy number variation under glutamine-limited growth, and two copy control (DGY1315) containing two copies of mCitrine in neutral loci. Samples of every population were taken every 8, 13, 8 generations (M, W, F). In each sample, 100,000 events were measured for mCitrine fluorescence (excitiaton wavelength = 516, emission wavelength = 529 ) via the B2-A channel, forward scatter, and side scatter. 
gap1_1, gap1_3, gap1_4 
gen 8 - 200

### Set working directory and get list of subdirectories
Place your FCS files in a folder. Multiple subdirectories containing FCS files are OK. 
...
### Version name
Choose a name to be used for all output files including the gating template and associated flow data and graphs. We will be using data from timepoints 1, 2, and 4 to draw gates. 
```{r}
version_name = "timepoints-1-2-4_v1"
```
...
Run the function on all subdirectories. Only needs to be run once to create the experimental details .csv files. 
```{r}
